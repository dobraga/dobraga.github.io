...
pega_lista_ep <- function(season=1){
  ...
  tag <- xml2::xml_find_all(page, '//*[@id="episodes_content"]/div[2]/div[2]/div/div[2]/strong/a')
...
Agora buscando as informações de todas as temporadas, temos:
...
pega_info_ep <- function(url){
  ...
  summarys <- paste0(
    ...
    xml2::read_html() %>% 
    xml2::xml_find_all('//*[@id="plot-summaries-content" or @id="plot-synopsis-content"]/li') %>% 
    xml2::xml_text()
...
Como temos mais de 200 linhas (episódios) seria muito demorado fazer a busca 
sequencialmente dessas informações, por isso, será feita uma busca em paralelo, 
para efeito de comparação a busca de 10 episódios sequencialmente demora por 
volta de:
...
Uma queda considerável no tempo de execução, agora buscando a inormações de 
todos os episódios:
